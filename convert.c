...
/* $Id: convert.c,v 1.7 2000-10-17 05:13:53 megastep Exp $ */
...
int convert_uninstall(const char *path, const char *name, const char *vername, const char *desc,
                      const char *uninstall, const char *update_url)
{
    ...
    if ( script ) {
        ...
        char url[PATH_MAX], curdir[PATH_MAX];
        ...
        if ( product ) {
            ...
            while ( ! feof(script) ) {
                ...
                if ( !strcmp(word, "rm") ) {
                    ...
                    str = remove_root(root, file);
                    if ( strcmp(str, root) ) { /* ... and the install directory */
...
{
    if ( argc < 5 ) {
        printf("Usage: %s /path/to/uninstall product version description [url]\n", argv[0]);
        return 1;
    }
    ...
    if ( check_uninstall(argv[1]) ) { 
        ...
        return convert_uninstall(argv[1], argv[2], argv[3], argv[4], uninst, argv[5]);
    } else {
